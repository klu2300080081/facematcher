...
class VGGNet(nn.Module):
    def __init__(self):
        ...
        # Modify final layer to output 2622 features
        self.vgg.classifier[6] = nn.Linear(4096, 2622)
        # Freeze feature layers to avoid retraining
...
def prepare_datasets(category_folders):
    ...
    # ... (unchanged dataset preparation logic)
...
def load_vgg_model():
    model = VGGNet()
    model.eval()
    st.success("Loaded pre-trained VGG16 model.")
    return model
...
def load_gender_model():
    ...
    if os.path.exists(model_path):
        ...
        return model
    st.error("Gender classifier model not found.")
    st.stop()
...
def generate_face_patterns(_vgg_model, male_df, female_df):
    ...
    if male_df_valid.empty and female_df_valid.empty:
        st.error("No valid image paths found in dataset.")
        st.stop()
...
def predict_celebrity(img, vgg_model, gc_model, male_patterns_df, female_patterns_df):
    ...
    if not min_similarity_row.empty:
        ...
        if os.path.exists(img_path):
            ...
            return None, instance['Name'], gender  # Fallback to name only
    else:
        if not df_patterns.empty:
            ...
            if os.path.exists(img_path):
                ...
                return None, best_match['Name'], gender
...
with col1:
    ...
    if st.session_state.captured_image_bytes:
        ...
        st.write("No image uploaded or captured. Please upload or capture an image.")
